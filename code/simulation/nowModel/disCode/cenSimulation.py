...
class Model():
    ...
    def storeData(self):
        with open(FileAdd+'/stateDictBeta'+str(self.beta)+'.csv', 'w') as fout:
            writer = csv.writer(fout)
            for s in self.State:
                writer.writerow(list(s)+State[s])
        with open(FileAdd+'/systemPerformanceBeta'+str(self.beta)+'.csv', 'w') as fout:
            writer = csv.writer(fout)
...
def writeResult(re):
    with open(FileAdd+'/varyBeta.csv', 'w') as fout:
        writer = csv.writer(fout)
        for i in range(Core):
            for j in range(3):
                for k in range(7):
                    writer.writerow([i*3+j, k]+re[i][j][k])
...
WriteFile = False
#WriteFile = True
FileAdd = 'C:/Rebalancing/nowModel/result/'+'A'+str(A)+'M'+str(M)+'Beta'
WarmTime = 500
RunTime = 3000
Core = 4
