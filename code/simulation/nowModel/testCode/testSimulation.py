...
class Model():
    ...
    def getRecord(self):
        #print('getRecord')
        #tPortn = (self.T - self.formerT) / self.timeLimit
        # self.brokenBikes += sum(self.state1[-3:]) * tPortn
        ...
        #self.normalBikes = (self.normalBikes*self.formerT+(sum(self.state1[:A]) + sum([sum(_) for _ in self.state2]))*(self.T-self.formerT))/self.T
        self.formerT = self.T
        return [self.T, self.lostCustomers/self.arrivals]
...
WriteFile = True
NAME = 'Beta'
FileAdd = 'testGetPerformance.csv'
WARMTIME = 0
RUNTIME = 20000
CORE = 4
NUMBERPERF = 7
