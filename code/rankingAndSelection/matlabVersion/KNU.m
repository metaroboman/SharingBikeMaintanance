...
%date: 6 May,2020
%%%%%%%%%%%
function [indexb,TotSS]=KNU(k,n0,Vdelta,muX,varX)
%% initilization
% k = 5;
% n0 = 24;
% b = 1;
...
I = ones(1,k);
Valpha = 0.05;
Veta = 1/2*((2*Valpha/(k-1))^(-2/(n0-1))-1);
h2 = 2*Veta*(n0-1);
X0 = zeros(n0,k);
S2 = zeros(k,k);
for i = 1:k
...
for i = 1:k
    for j = 1:k
        S2(i,j) = 1/(n0-1)*sum((X0(:,i)-X0(:,j)-ones(n0,1).*(hat_muX(i)-hat_muX(j))).^2);
    end
end
barX = hat_muX; % sample mean
for i = 1 : k
    for j = 1 : k
        if  i ~= j && I(i)>0 && I(j)>0 && barX(j) - barX(i) <= -Boundary(Vdelta,h2,S2(i,j),r)
            I(j) = 0;
        end
    end
end
