...
class Model():
        ...
        def getState(a, m):
            ...
            def generate_R(s, index):
                if len(s) == A + 3 + A ** 2 and sum(s) == M:
                    State[tuple(s+[0])] = index[0]
                    State[tuple(s+[1])] = index[0]+1
                    index[0] += 2
    ...
    def getMtx(self):
        ...
        def IBP(s):
            if s[-4]>0 and s[-1] == 0: return 1
            else: return 0
        def IDP(s):
            if s[-2] >0 and s[-1] == 1: return 1
            else: return 0
        def INi(ni):
            if ni > 0: return 1
            else: return 0
        ...
        def gathRateIn(s):
            ...
            if s[-1] == 1:
                for i in range(self.C+1):
                    tempS = list(s)
                    if i<self.C and tempS[-4]>0: continue
                    tempS[-4] += i
                    tempS[-3] -= i
                    tempS[-1] = 0
                    #if tempS[-4] == 0: continue
                    if IS(tempS):
                        n += 1
                        self.assertion(tempS)
                        col.append(State[tuple(tempS)])
        ...
        def fixRateIn(s):
            ...
            tempS = list(s)
            # RC, DP
            a1 = tempS[-3] = tempS[-3] + 1
            a2 = tempS[-2] = tempS[-2] - 1
            if IS(tempS):
                n += 1
                self.assertion(tempS)
                col.append(State[tuple(tempS)])
        ...
        def redRateIn(s):
            ...
            if s[-1] == 0:
                for i in range(self.C+1):
                    tempS = list(s)
                    if i<self.C and tempS[-2]>0: continue
                    tempS[-2] += i
                    #if tempS[-2] == 0: continue # 如果待投放量为0，则不可能发生投放
                    tempS[-1] = 1
                    result_lst, tail = [], tempS[self.A:]
                    arr_rank, num_dis, N_lst1, dbar1 = \
                        np.argsort(self.ArrLst)[::-1], self.num_dis, tempS[:self.A], i
        ...
        def getRateOut(s):
            ...
            outRate += self.Mu * min(s[-3], self.N)+ self.Tau# + IBP(s)*self.Tau+IDP(s)*self.Tau
    ...
    def getPerf(self):
        ...
        for k, s in enumerate(portionState):
            por = portionState[s]
            normalBikes += sum(s[:-4]) * por
            brokenBikes += sum(s[-4:-1]) * por
            if s[-3] == 0: idle += por
            BP += s[-3] * por
            RC += s[-2] * por
            DP += s[-1] * por
...
def writeResult(re):
    ...
    df.to_csv(FileAdd+'TauOdeSingle.csv')
