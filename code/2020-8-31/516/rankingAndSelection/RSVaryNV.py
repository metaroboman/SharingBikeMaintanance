...
#                         plt.pause(0.000000000001)
        ...
        #e = time.time()
        #print('-'*30, '\n', e-s)
        return np.mean(self.hl[1][-1000:]),np.std(self.hl[1][-1000:])
...
def run(p):
    ...
    for epi in tqdm(range(REPEAT)):
        ...
        for smt in np.arange(2*p+1, 2*(p+1)+1,1):
        #for i in range(1):
            #smt = 0.2
            #smt = round(smt, 1)
            if smt > 7: continue
            N = smt
            for j in range(1,(Total - cr*N)//cc+1):
                para = V = j
                #para = V = smt
...
def writeResult(re):
    ...
    df.to_csv(FileAdd+'RSMulticarTotal50test.csv')
